...
class OrdersAdapter(private val orders: List<Order>) : RecyclerView.Adapter<OrdersAdapter.OrderViewHolder>() {
    ...
    inner class OrderViewHolder(private val binding: ItemOrderBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        fun bind(order: Order) {
            // Siparişin tüm item'lerini göstermek için items listesine bakıyoruz
            val item = order.items.first()  // İlk ürünü alıyoruz (isteğe göre hepsini de listeleyebilirsiniz)
            ...
            binding.textViewProductQuantity.text = "Quantity: ${item.count}"
            binding.textViewProductPrice.text = "Price: ${item.price} TL"
            binding.textViewOrderDate.text = "Order Date: ${order.timestamp}"

            // Ürün görseli için Glide kullanımı
            Glide.with(binding.imageViewProduct.context)
                .load(item.image)  // siparişin ürün görselini alıyoruz
                .into(binding.imageViewProduct)
        }
    }
}
